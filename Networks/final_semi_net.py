...
class Net(nn.Module):
    ...
    def forward(self,moving,fixed,mov_seg):

        fix_enc = self.Encoder(fixed)
        mov_enc = self.Encoder(moving)
        
        pre_mov_seg,mseg_feat = self.Mor_Decoder(mov_enc)
        pre_fix_seg,fseg_feat = self.Mor_Decoder(fix_enc)
        
        flow_1,flow_5,out4,out3,out2,out1,SVF_1 = self.Reg_Decoder(mov_enc,fix_enc,fseg_feat,mseg_feat)
        if self.for_train:
            warped1 = self.STN(moving, flow_1)
            warped_mov_seg = self.NearSTN(mov_seg,flow_1)
            warpeed_pre_mov_seg = self.STN(pre_mov_seg,flow_1)
            
            return flow_1,warped1,warped_mov_seg,warpeed_pre_mov_seg,pre_mov_seg,pre_fix_seg
        else:
            return flow_1,pre_mov_seg,pre_fix_seg
...
class Morphology_decoder(nn.Module):
    def __init__(self, 
        ...
        self.seghead = SegHead_block(channel_num*2, 4, use_checkpoint)
    ...
    def forward(self,x_enc):
        ...
        pre_seg = self.seghead(y4)   # 1,4,160,,
...
class Registration_decoder(nn.Module):
    ...
    def forward(self,m_enc,f_enc,fseg_feat,mseg_feat):
        ...
        return flow_1,flow_5,out4,out3,out2,out1,SVF_1
