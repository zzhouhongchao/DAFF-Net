...
from Networks import final_uns_net
...
def train(args):
    ...
    model = final_uns_net.Net(for_train=True,use_checkpoint=True)
    model.to(device)
    ...
    for i in range (1,args.epoch + 1 ):
        for data in DL:
            ...
            flow_1,warped1= model(input_moving,input_fixed)
            ...
            loss = sim_loss1 + grad_loss1 + 0.00001 * NJ_loss1 
            
            print("i: %d name: %s loss: %f sim: %f L2: %f njd: %f "
                  % (i, name, loss.item(), sim_loss1.item(), grad_loss1.item(),NJ_loss1.item()),flush=True)                                                                                                         
            print("%d, %s, %f, %f, %f,%f"
                  % (i, name, loss.item(), sim_loss1.item(), grad_loss1.item() ,NJ_loss1.item()),file=f)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--model_dir", type=str, help="data folder with training vols",
                    dest="model_dir", default="model/newexperiments/OASIS3_uns")
    parser.add_argument("--Log_dir", type=str, help="data folder with training vols",
                    dest="Log_dir", default="logs/newexperiments/OASIS3_uns")
